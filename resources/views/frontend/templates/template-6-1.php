...
foreach ($AllProductsData as $key => $value) {
    ...
    $img = esc_url($catalog->portfolio_catalog_get_image_by_sizes_and_src($imgurl[0], array(
        150,
        150
    ), true));
    $thumbnails = array();
    ...
    ?>
    <div class="shwcgrid-item <?php echo $catClass; ?> " data-category="<?php echo esc_html(str_replace("-","",$catClass)); ?>">
        <script>
        ...
        <ul class="images_list">
            ...
            <?php foreach ($thumbnail as $key => $val) {
                ...
                ?><?php if ($thumbnailUrl != null) {
                    ...
                    $img11 = esc_url($catalog->portfolio_catalog_get_image_by_sizes_and_src($imgurl[0], array(
                        150,
                        150
                    ), true));
                    $imgurlThumb1 = explode(";", $thumbnailUrl);
                    $imgurlThumb150 = esc_url($catalog->portfolio_catalog_get_image_by_sizes_and_src($imgurlThumb1[0], array(
                        150,
                        150
                    ), true));
                    ?>
                    <li onclick="selectProduct_(this, <?= $val->id; ?>, <?= $prodId; ?>)" imgurlmain1024="<?php echo $imgurlThumb1[0]; ?>" selectedUrl="<?php echo $imgurlThumb1[0]; ?>">
                        <div class="image_block">
                            <div class="bg_wrapper" style="background-image:url(<?php echo $imgurlThumb150; ?>)"></div>
                            <img src="<?php echo $imgurlThumb150; ?>" ns='<?php echo $ns; ?>' data-trackId='<?php echo $index; ?>'/>
                        </div>
                    </li>
                <?php }
            } ?>
        ...
        <div class="info_block">
            ...
            <div class="price_wrapper" onclick="product(this, '<?php echo $productUrl ?>', '<?php echo $productUrl ?>', '<?php echo $imgurlmain1024; ?>')">
