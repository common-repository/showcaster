...
.shwc_pc_wrapper.pc_view3 .shwcgrid-item {
    ...
    height:<?php echo $gridItemHeight+12; ?>px;
<?php
...
@media (max-width: 992px) {
    .shwc_pc_wrapper.pc_view3  .shwcgrid-item {
        min-width:calc(33% - <?php echo $gridItemMargin*2; ?>px);
    }
}

@media (max-width: 768px) {
    .shwc_pc_wrapper.pc_view3  .shwcgrid-item {
        width:calc(50% - <?php echo $gridItemMargin*2; ?>px);
    }
}
...
@media (max-width: 640px) {
    .shwc_pc_wrapper.pc_view3  .shwcgrid-item {
        width:90% !important;
        max-width:<?php echo $layoutWidth; ?>% !important;
        height:auto;
    }
}
...
.shwc_pc_wrapper.pc_view3 .shwcgrid-item .title {
...
.shwc_pc_wrapper.pc_view3 .shwcgrid-item:hover .title { }
...
<?php } else {$dh=($gridItemDescription1+2)*2;}?>
    ...
    height:<?php echo ($gridItemDescription1+2)*3; ?>px;
...
.shwc_pc_wrapper.pc_view3 .shwcgrid-item:hover .description {color:#;}
.shwc_pc_wrapper.pc_view3 .shwcgrid-item .price {
<?php
 $ph=0;
...
.shwc_pc_wrapper.pc_view3 .shwcgrid-item:hover .price { color:#; }
.shwc_pc_wrapper.pc_view3 .shwcgrid-item .discont_price {
<?php if($gridItemShowDiscountPrice!="true"){ ?> display:none;
<?php } else {
    if($ph<($gridItemShowDiscountPrice1+2)){$ph=$gridItemShowDiscountPrice1+7;}
...
.shwc_pc_wrapper.pc_view3 .shwcgrid-item:hover .discont_price { color:#; }

.shwc_pc_wrapper.pc_view3 .shwcgrid-item .info_block {
<?php if($th>0 || $dh>0 || $ph>0){ ?> height:<?php echo $th+$dh+$ph+12;?>px; <?php } ?>
}



.shwc_pc_wrapper.pc_view3 .shwcgrid-item .board_block {
<?php if($th>0 || $dh>0 || $ph>0){ ?>
    height:<?php echo $gridItemHeight-($th+$dh+$ph+12);?>px;
<?php }else { ?>
